...
uploaded_files = st.file_uploader(
    "Upload Screaming Frog export(s)",
    type=["csv", "xls", "zip"],
    accept_multiple_files=True,
    help="Upload one or more CSV/XLS files, or a ZIP. Select multiple files with Cmd+click.",
)
...
with st.spinner("Reading files..."):
    ...
    for uploaded in uploaded_files:
        try:
            ...
            if uploaded.name.lower().endswith(".zip"):
                frames.append(load_zip_of_csvs(file_bytes))
            else:
                frames.append(load_single_file(file_bytes, uploaded.name))
    ...
    # Merge all frames — outer join on Address to combine columns across files
    df = frames[0]
    for frame in frames[1:]:
        if frame.empty:
            continue
        if "Address" in df.columns and "Address" in frame.columns:
            new_cols = [c for c in frame.columns if c not in df.columns and c != "Address"]
            if new_cols:
                df = df.merge(frame[["Address"] + new_cols], on="Address", how="outer")
            else:
                df = pd.concat([df, frame]).drop_duplicates(subset=["Address"]).reset_index(drop=True)
        else:
